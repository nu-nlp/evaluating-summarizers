#!/usr/bin/env python
# -*- coding: utf-8 -*-
# SOURCE: https://github.com/UKPLab/coling2016-genetic-swarm-MDS/blob/master/JS.py
...
def get_all_content_words_lemmatized(sentences, N=1):
    ...
    for s in sentences:
        all_words.extend(
            [wordnet_lemmatizer.lemmatize(r) for r in tokenizer.tokenize(s)]
        )
    if N == 1:
...
def get_all_content_words_stemmed(sentences, N=1):
    ...
    if N > 1:
        return [
            gram
            for gram in ngrams(normalized_content_words, N)
            if is_ngram_content(gram)
        ]
    return normalized_content_words
...
def compute_tf_doc(docs, N=1):
    ...
    content_word_tf = dict(
        (w, f / float(len(word_freq.keys()))) for w, f in word_freq.items()
    )
...
def compute_tf(sentences, N=1):
    ...
    content_word_tf = dict(
        (w, f / float(content_words_count)) for w, f in content_words_freq.items()
    )
    return content_word_tf
...
def js_divergence(sys_summary, doc_freq):
    ...
    jsd = kl_divergence(summary_freq, average_freq) + kl_divergence(
        doc_freq, average_freq
    )
    return jsd / 2.0
